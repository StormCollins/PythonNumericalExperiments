...
class CVA:
    ...
    def compute_cva(self, tenors, price_paths):
        discount_factors = self.interest_rate_curve.get_zero_rates(tenors)
        default_probabilities = self.survival_curve.get_probabilities_of_default(tenors)
        return np.average(np.sum((1 - self.survival_curve.recovery_rate) * np.maximum(price_paths, 0)*discount_factors*default_probabilities, 1))